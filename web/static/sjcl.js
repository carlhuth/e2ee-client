 ...
 sjcl.json = {
  ...
  _encrypt: function (password, plaintext, params, rp) {
    ...
    if (p.mode === "ccm" && sjcl.arrayBuffer && sjcl.arrayBuffer.ccm && plaintext instanceof ArrayBuffer) {
      ...
      p.ciphertextBuffer = c["ciphertext_buffer"]
      p.ciphertextTag = c["tag"]
    } else {
  ...
  _decrypt: function (password, ciphertext, params, rp) {
    ...
    if (p.mode === "ccm" && sjcl.arrayBuffer && sjcl.arrayBuffer.ccm && 
	  ...
	  // if plaintext pushed into encryption function was of type ArrayBuffer, then the
	  // ciphertext was calculated as ArrayBuffer and encoded into Base64 (ciphertextBuffer),
	  // tag was calculated as bitArray and encoded into Base64 (ciphertextTag)
...
sjcl.ecc.deserialize = function (key) {
  ...
  if (key.secretKey) {
    ...
    var point = curve.fromBits(sjcl.codec.base64.toBits(key.point));
  ...
  publicKey: function(curve, point) {
    ...
    this.serialize = function () {
      ...
      return {
        type: this.getType(),
        secretKey: false,
        point: sjcl.codec.base64.fromBits(this._point.toBits()),
        curve: curveName
      };
    };
...
sjcl.arrayBuffer.ccm = {
  ...
  encrypt: function(prf, plaintext_buffer, iv, adata, tlen, ol){
    ...
    encrypted_obj = {'ciphertext_buffer':plaintext_buffer.slice(0, ol), 'tag':mac};
...
sjcl.codec.arrayBuffer = {
  ...
  // lookup table to find the index of certain character (actually character's unicode) in _chars

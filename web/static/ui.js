								...
								if(err){
									...
									Object.keys(messages).forEach(function (key) {
										...
										if (message.payload.operation === 'share'){
											...
											files.push({'fileName': fileName, 'peer': peerName, 'access': access, 'hmac': hmac})
										} else {
											var fileName = message.payload.fileName
				...
				container.share(peer, function (err) {
					...
					e2ee.session.indexContainer.get('fileNames', function(err, fileNames){
						if(err){
							...
							sharedFiles.push({'fileName': file, 'access': access, 'user': user})
							e2ee.session.indexContainer.save(function(err){
	                    	    if(err){
	                    	     	if (window.console && window.console.log) {
	                    	        	console.error(err)
...
$('#download').on('click', function() {
	...
	var files = e2ee.UI.getCheckedFiles()
	console.info(files)
	async.each(files, e2ee.crypto.getFile, function (err) {	
	})
	return false
})
...
e2ee.UI.preparePanels = function(username) {
	...
	container.get('fileNames', function(err, fileNames){
		if(err){
			...
			var sharedFileNames = jQuery.map(fileNames['sharedFiles'], function(b){return b.fileName})
			for (var i = 0; i < files.length; i++) { 
				var fileName = files[i]
				var shared = false
				...
				e2ee.UI.addFileElement(fileName, shared, true)
			}
			files = fileNames['notMineFiles']
			for (var i = 0; i < files.length; i++) { 
				var file = files[i]
				e2ee.UI.addFileElement(file.fileName, true, false)
			}
...
e2ee.UI.addFileElement = function(fileName, shared, isMyContainer) {
	...
	var htmlElement = '<div class="fileElement">' + c + '<div class="fileInfo"><p>'+fileName+'</p><span>added 21.5.2015</span></div><img src="static/icons/' + icon + '" alt="shared" height="'+iconSize+'" width="'+iconSize+'"></div>'
...
e2ee.UI.addFile = function(fileName, alreadyExists) {
	if (!alreadyExists){
		e2ee.UI.addFileElement(fileName, false, true)
	}
}
